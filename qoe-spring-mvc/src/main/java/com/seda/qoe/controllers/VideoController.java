...
public class VideoController {
	...
	public String startTesting(@Valid @ModelAttribute("questionaryCreate") QuestionaryCreateDTO formBean, BindingResult bindingResult, Model model, RedirectAttributes redirectAttributes,
		...
		QuestionaryDTO questionary = questionaryFacade.create(q);
		VideoDTO video = videoFacade.findRandomVideo();
		model.addAttribute("questionaryObj", questionary);
		model.addAttribute("videoObj", video);
		model.addAttribute("scenarioObj", video.getScenario().get((int) Math.random() * video.getScenario().size()));
...
//		System.out.println("Scenario id je: " + video.getScenario().get((int) Math.random() * video.getScenario().size()).getId());
...
//		System.out.println("Questionary id:" + ((QuestionaryDTO) session.getAttribute("questionaryObj")).getId());
//		System.out.println("ID videa je: " + video.getId());
//		System.out.println("Scenario id je: " + video.getScenario().get((int) Math.random() * video.getScenario().size()).getId());
		return "testvidea/video";
	}
